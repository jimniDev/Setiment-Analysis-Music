...
export function Card ({img}) {
    ...
    const [position, setPosition] = useState({ x: 0, y: 0, deg: 0, duration: 0});
    ...
    function onPointerMove(e){
        // if (isDown){
            setPosition({
                ...
                x: e.clientX - start.x,
                y: e.clientY - start.y,
                deg: (e.clientX - start.x) / innerWidth * 50,
            })
    ...
    function onPointerUp() {
        ...
        console.log(position.x, cardRef.current.clientWidth / 2)
        cardRef.current.removeEventListener('pointerdown', onPointerDown)
        
        if (Math.abs(position.x) > cardRef.current.clientWidth / 2) {
    ...
    return (
        ...
        <div ref={cardRef}
            ...
            style={{
                ...
                transform : `translate3d(${position.x}px, ${position.y}px, 0) rotate(${position.deg}deg)`,
                transition : `transform ${position.duration}ms`
            }}
